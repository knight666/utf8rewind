...
TEST_F(Utf8ToTitleGreek, SingleCapitalLetterSigma)
{
	// 03A3
	// 03A3
...
TEST_F(Utf8ToTitleGreek, SingleSmallLetterSigma)
{
	// 03C3
	// 03A3
...
TEST_F(Utf8ToTitleGreek, SingleSmallLetterFinalSigma)
{
	// 03C2
	// 03A3
...
TEST_F(Utf8ToTitleGreek, WordCapitalLetterSigma)
{
	// 03AB 03A3 039E 039A
	// 03AB 03C3 03BE 03BA
	...
	EXPECT_EQ(8, utf8totitle(c, strlen(c), b, s - 1, &errors));
	EXPECT_UTF8EQ("\xCE\xAB\xCF\x83\xCE\xBE\xCE\xBA", b);
	EXPECT_ERROREQ(UTF8_ERR_NONE, errors);
}

TEST_F(Utf8ToTitleGreek, WordCapitalLetterSigmaAtEnd)
{
	// 03A3 039F 0399 03A3
	// 03A3 03BF 03B9 03C2
...
TEST_F(Utf8ToTitleGreek, WordSmallLetterSigma)
{
	// 03B5 03BB 03C0 03C3
	// 0395 03BB 03C0 03C3
...
TEST_F(Utf8ToTitleGreek, WordSmallLetterFinalSigma)
{
	...
	EXPECT_EQ(10, utf8totitle(c, strlen(c), b, s - 1, &errors));
	EXPECT_UTF8EQ("\xCE\x9E\xCE\xB2\xCE\xBF\xCE\xAC\xCF\x82", b);
	EXPECT_ERROREQ(UTF8_ERR_NONE, errors);
}

TEST_F(Utf8ToTitleGreek, SentenceCapitalLetterSigma)
{
...
TEST_F(Utf8ToTitleGreek, SentenceSmallLetterSigma)
{
	...
	EXPECT_EQ(20, utf8totitle(c, strlen(c), b, s - 1, &errors));
	EXPECT_UTF8EQ("\xCE\xA1\xCE\xBC\xCE\xBE\xCF\x83 \xCE\xA1\xCF\x83\xCF\x86 \xCE\x99\xCE\xB1", b);
	EXPECT_ERROREQ(UTF8_ERR_NONE, errors);
}

TEST_F(Utf8ToTitleGreek, SentenceSmallLetterFinalSigma)
{
