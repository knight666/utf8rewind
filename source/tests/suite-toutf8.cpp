...
TEST(ToUtf8, Ucs2OneByte)
{
	...
	EXPECT_EQ(1, wctoutf8((const wchar_t*)&c, 2, b, s));
	EXPECT_STREQ("*", b);
}
...
TEST(ToUtf8, Ucs2TwoBytes)
{
	...
	EXPECT_EQ(2, wctoutf8((const wchar_t*)&c, 2, b, s));
...
TEST(ToUtf8, UnmatchedLowSurrogatePair)
{
	...
	EXPECT_EQ(UTF8_ERR_UNMATCHED_LOW_SURROGATE_PAIR, wctoutf8((const wchar_t*)c, 4, b, s));
	EXPECT_STREQ("", b);
}
...
TEST(ToUtf8, UnmatchedHighSurrogatePair)
{
	...
	EXPECT_EQ(UTF8_ERR_UNMATCHED_HIGH_SURROGATE_PAIR, wctoutf8((const wchar_t*)c, 4, b, s));
	EXPECT_STREQ("", b);
}
...
TEST(ToUtf8, NotEnoughSpace)
{
	...
	EXPECT_EQ(UTF8_ERR_NOT_ENOUGH_SPACE, wctoutf8(c, 2, b, s));
	EXPECT_STREQ("", b);
}
...
TEST(ToUtf8, InvalidLength)
{
	...
	EXPECT_EQ(UTF8_ERR_INVALID_DATA, wctoutf8((const wchar_t*)c, 1, b, s));
	EXPECT_STREQ("", b);
}
