...
TEST(ToUtf8, Convert)
{
	...
	EXPECT_EQ(9, wctoutf8(c, sizeof(c), b, s, &errors));
	EXPECT_EQ(0, errors);
...
TEST(ToUtf8, OneByte)
{
	...
	EXPECT_EQ(1, wctoutf8(&c, sizeof(c), b, s, &errors));
	EXPECT_EQ(0, errors);
	EXPECT_STREQ("*", b);
}
...
TEST(ToUtf8, TwoBytes)
{
	...
	EXPECT_EQ(2, wctoutf8(&c, sizeof(c), b, s, &errors));
	EXPECT_EQ(0, errors);
...
TEST(ToUtf8, UnmatchedLowSurrogatePair)
{
	...
	EXPECT_EQ(0, wctoutf8(c, sizeof(c), b, s, &errors));
...
TEST(ToUtf8, InvalidLength)
{
	...
	EXPECT_EQ(0, wctoutf8(c, 1, b, s, &errors));
	EXPECT_EQ(UTF8_ERR_INVALID_DATA, errors);
	EXPECT_STREQ("", b);
}
...
TEST(ToUtf8, OutputLengthOneByte)
{
	...
	EXPECT_EQ(1, wctoutf8(c, sizeof(c), nullptr, 0, &errors));
	EXPECT_EQ(0, errors);
}
...
TEST(ToUtf8, OutputLengthTwoBytes)
{
	...
	EXPECT_EQ(2, wctoutf8(c, sizeof(c), nullptr, 0, &errors));
	EXPECT_EQ(0, errors);
}
...
TEST(ToUtf8, OutputLengthThreeBytes)
{
	...
	EXPECT_EQ(3, wctoutf8(c, sizeof(c), nullptr, 0, &errors));
	EXPECT_EQ(0, errors);
}
...
TEST(ToUtf8, OutputLengthFourBytes)
{
	...
	EXPECT_EQ(4, wctoutf8(c, sizeof(c), nullptr, 0, &errors));
	EXPECT_EQ(0, errors);
}
