...
TEST(ToTitle, GeneralCategoryCaseMappedMultipleUppercase)
{
	...
	EXPECT_EQ(7, utf8totitle(c, strlen(c), b, s - 1, 0, &errors));
...
TEST(ToTitle, GeneralCategoryCaseMappedSentence)
{
	...
	EXPECT_EQ(31, utf8totitle(c, strlen(c), b, s - 1, 0, &errors));
...
TEST(ToTitle, InvalidCodepointAmountOfBytes)
{
	...
	EXPECT_EQ(9, utf8totitle(c, strlen(c), nullptr, 0, 0, &errors));
	EXPECT_EQ(0, errors);
}
